        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if (partyData[i].nature > 0)
            {
                if (gSaveBlock2Ptr->optionsDifficultyMode == OPTIONS_DIFFICULTY_EASY)
                    CreateMonWithGenderNatureLetter(&party[i], partyData[i].species, partyData[i].lvl - 1, NULL, gender, partyData[i].nature, 0, partyData[i].shiny ? OT_ID_RANDOM_SHINY : OT_ID_RANDOM_NO_SHINY);
                else if (gSaveBlock2Ptr->optionsDifficultyMode == OPTIONS_DIFFICULTY_HARD)
                    CreateMonWithGenderNatureLetter(&party[i], partyData[i].species, partyData[i].lvl + 1, NULL, gender, partyData[i].nature, 0, partyData[i].shiny ? OT_ID_RANDOM_SHINY : OT_ID_RANDOM_NO_SHINY);
                else
                    CreateMonWithGenderNatureLetter(&party[i], partyData[i].species, partyData[i].lvl, NULL, gender, partyData[i].nature, 0, partyData[i].shiny ? OT_ID_RANDOM_SHINY : OT_ID_RANDOM_NO_SHINY);
            }
            else
            {
                if (gSaveBlock2Ptr->optionsDifficultyMode == OPTIONS_DIFFICULTY_EASY)
                    CreateMon(&party[i], partyData[i].species, partyData[i].lvl - 1, NULL, TRUE, personalityValue, partyData[i].shiny ? OT_ID_RANDOM_SHINY : OT_ID_RANDOM_NO_SHINY, 0);
                else if (gSaveBlock2Ptr->optionsDifficultyMode == OPTIONS_DIFFICULTY_HARD)
                    CreateMon(&party[i], partyData[i].species, partyData[i].lvl + 1, NULL, TRUE, personalityValue, partyData[i].shiny ? OT_ID_RANDOM_SHINY : OT_ID_RANDOM_NO_SHINY, 0);
                else // (gSaveBlock2Ptr->optionsDifficultyMode == OPTIONS_DIFFICULTY_MED)
                    CreateMon(&party[i], partyData[i].species, partyData[i].lvl, NULL, TRUE, personalityValue, partyData[i].shiny ? OT_ID_RANDOM_SHINY : OT_ID_RANDOM_NO_SHINY, 0);
            }
